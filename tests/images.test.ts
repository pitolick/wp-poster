...
describe('downloadImage', () => {
  ...
  it('クエリストリング付きでもファイル名を抽出できる', async () => {
    ...
    const img = await downloadImage('https://example.com/path/cover.webp?foo=bar', { fetch: fetchMock });
  ...
  it('ダウンロード→アップロード→media id を返す', async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init: RequestInit) => {
      ...
      if (url.includes('/wp/v2/media')) {
        return new Response(JSON.stringify({ id: 555, source_url: 'https://wp/x.jpg', media_type: 'image' }), {
